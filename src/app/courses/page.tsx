...
const CoursePage = () => {
  ...
  const handleAction = {
    enroll: (id: string) => console.log("Enroll:", id),
    teacher: (id: string) => console.log("Teacher:", id),
    wishlist: (id: string) => setWishlist(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]),
    share: (id: string) => {
      if (navigator.share) {
        navigator.share({ title: "Check out this course!", url: `${window.location.origin}/course/${id}` });
      }
    },
  };
      ...
      <header className="bg-white border-b shadow-sm">
        <div className="container mx-auto px-4 py-6 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            {[categories, levels, sortOptions].map((list, i) => (
              <Select key={i} value={[selectedCategory, selectedLevel, sortBy][i]} onValueChange={val => [setSelectedCategory, setSelectedLevel, setSortBy][i](val)}>
                <SelectContent>
                {list.map(item => (
        ...
        {filteredCourses.length === 0 ? (
          <div className="text-center max-w-md mx-auto">
            <Card className="p-8">
              ...
              <Button variant="outline" onClick={() => { setSearchQuery(""); setSelectedCategory("All"); setSelectedLevel("All"); }}>Clear all filters</Button>
            </Card>
          </div>
        ) : (
          <>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-semibold text-gray-900">{searchQuery ? `Results for "${searchQuery}"` : "All Courses"}</h2>
              <div className="hidden sm:flex items-center text-sm text-gray-500"><Filter className="w-4 h-4 mr-1" />Sorted by {sortBy}</div>
            </div>
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.slice(1).map(cat => (
              <CategoryCard key={cat} name={cat} count={courses.filter(c => c.category === cat).length} onClick={() => setSelectedCategory(cat)} />
            ))}
          </div>
        </div>
      </section>
